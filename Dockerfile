...
RUN tar -xvf latest-fr_FR.tar.gz && rm latest-fr_FR.tar.gz
RUN wget https://files.phpmyadmin.net/phpMyAdmin/5.1.0/phpMyAdmin-5.1.0-all-languages.tar.gz
RUN tar -xvf phpMyAdmin-5.1.0-all-languages.tar.gz && rm phpMyAdmin-5.1.0-all-languages.tar.gz
RUN mv phpMyAdmin-5.1.0-all-languages phpmyadmin
RUN cp /srcs/wp-config.php /var/www/html/wordpress && cp /srcs/config.inc.php /var/www/html/phpmyadmin
WORKDIR /
